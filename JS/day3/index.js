...
let arrFromJson = JSON.parse(employeeData);
// 1.Ստեղծել նոր զանգված հետևյալ մոդելով { name , position, salary };

let arr1 = arrFromJson.map((i) => ({ name: i.name, position: i.hr.position, salary: i.hr.salary }))
console.log(arr1);
...
let employee = [];
arrFromJson.map((i,index) => {
    if(Number(i.hr.salary.replace("$",'').replace(",",'')) >= 150000){
       employee[index] = arrFromJson[index]
...
let employee1 = [];
arrFromJson.forEach((i,index) => {
    if(Number(i.hr.salary.replace("$",'').replace(",",'')) <= 100000){
        employee1[index] = arrFromJson[index]
        employee1[index].hr.bonus = "20%"
...
let objSities = {};
let sities = arrFromJson.map((i) => i.contact[0])
sities.forEach((i) => objSities[i] = objSities[i] ? objSities[i]+ 1 : 1 )

console.log(objSities)
...
function DismissEmployee (arrFromJson){

    console.log("BOSS: Սպասեք մտածեմ")

    setTimeout(() => {

    let emploes = arrFromJson.map((i) => i.name )
    console.log("Մեր բոլոր աշխատակիցները։  " , emploes);
    let randNumber = Math.floor(Math.random() * arrFromJson.length);
    let deletedEmploer = arrFromJson.splice(randNumber, 1);
    console.log("Ներող ապեր քեզ դուրս ենք հանում: ", deletedEmploer[0].name + "  from " + deletedEmploer[0].contact[0])
    let aswEmploes =  arrFromJson.map((i) => i.name );
    console.log("Նոր աշխատակիցների ցուցակ։ " , aswEmploes)

    },3000)

}

DismissEmployee (arrFromJson)